...
predictAndEvaluate <- function(rf, testData, ntrees){
    testPred <- c()
    for (i in 1:ntrees) {
        testPred <- cbind(testPred, predict(rf[[i]], testData)[,"alive"])
    }
    avgPred <- rowMeans(testPred)
    return(getBalancedAcc(avgPred, testData$deadOrAlive))
}
